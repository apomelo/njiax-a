...
public class IaxPhone implements PeerListener {
    ...
    public IaxPhone(String user, String pass, String host, boolean register,
                    int maxCalls) {
        peer = new Peer(this, user, pass, host, register, maxCalls);
    }

    public void answered(String calledNumber) {
        System.out.println(sdf.format(new Date()) + "   IAXphone, answered: " + calledNumber);
    ...
    public void hungup(String calledNumber) {
        System.out.println(sdf.format(new Date()) + "   IAXphone, hungup: " + calledNumber);
        long gapTime = System.currentTimeMillis() - answeredTime;
        System.out.println("the gap time is " + gapTime/1000 + " second.");
...
//        answerCall();
    ...
    public void sendDTMF(char tone) {
        System.out.println("IAXphone, sendDTMF: " + tone);
    ...
    public static void main(String[] args) {
        ...
        IaxPhone iaxPhone = new IaxPhone(user, pass, host, register, maxCalls);
        ...
        try {
            ...
            while ((line = stdin.readLine()) != null) {
                words = line.split(" ");
                if ((words[0].equals("call") || words[0].equals("c")) &&
                        words.length == 2) {
                    iaxPhone.makeCall(words[1]);
                }else if ( words[0].equals("callImmediately") &&
                        words.length == 1) {
                    iaxPhone.makeCall("µç»°ºÅÂë");
                    ...
                    iaxPhone.answerCall();
                        ...
                        words[0].equals("*") || words[0].equals("#") ) {
                    iaxPhone.sendDTMF(words[0].charAt(0));
